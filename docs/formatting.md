...
Wide figures/tables may run across both columns, and may combine several subfigures or tables. Figures and tables must be at least one column wide; letting a column of text wrap around a figure or table (e.g., using the wrapfig package) is not permitted.
